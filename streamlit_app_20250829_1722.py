...
if not DEEPSEEK_KEY:
    ...
    def generate_cluster_description(cluster_id, cluster_data, mode, bin_present):
        """Generuje opis klastra z cache'owaniem"""
        try:
            if mode == "Analiza":
                sys_prompt = (
                    "Jesteś doświadczonym analitykiem danych. Przeanalizuj tę grupę użytkowników "
                    "i podaj rzeczowy, techniczny opis. Skup się na statystykach i wzorcach. "
                    "Odpowiedź po polsku, max 8 zdań."
                )
                temperature = 0.3
                max_tokens = 400
            else:
                sys_prompt = (
                    "Jesteś dowcipnym obserwatorem społeczności. Opisz tę grupę użytkowników "
                    "w lekki, ironiczny sposób. Baw się stereotypami IT. "
                    "Odpowiedź po polsku, max 3 zdania."
                )
                temperature = 0.8
                max_tokens = 200
            ...
            user_prompt = f"""
            Opisz grupę {int(cluster_id)} liczącą {len(cluster_data)} osób.
            Najważniejsze cechy grupy (procent osób z daną cechą):
            {', '.join([f'{r['feature']}: {r['share_of_1_pct']}%' for r in preview_rows[:8]])}
            """

            # Wywołanie API
            body = {
                "model": DEEPSEEK_MODEL,
                "stream": False,  # Bez streamingu dla cache
    ...
    if st.button("🎯 Generuj opis klastru", type="primary"):
        with st.spinner("🔮 DeepSeek analizuje grupę..."):
            description = generate_cluster_description(
                cluster_desc, cluster_data, mode, bin_present
            )
        ...
        if description.startswith(("❌", "⏰", "🔑", "💳", "🚫", "⚠️")):
            ...
            st.caption(f"🤖 Wygenerowano dla grupy {cluster_desc} ({mode.lower()} mode)")
